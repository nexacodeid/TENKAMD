...
try {
  cachedThumb = getAssetBuffer("haidar2");
} catch {}
...
async function startAutoJpmSession(m, sock, db) {
  ...
  return sendInteractiveMessage(m, sock, {
    title: `🔄 ${config.bot?.name || "Haidar-AI"} AutoJPM`,
    body,
    footer: `${config.bot?.name || "Haidar-AI"} AutoJPM`,
    buttons,
  });
}
...
async function handleSetDelay(m, sock, db, input) {
  ...
  if (!input) {
    ...
    return sendInteractiveMessage(m, sock, {
      title: `⏱️ ${config.bot?.name || "Haidar-AI"} Delay`,
      body,
      footer: `${config.bot?.name || "Haidar-AI"} JPM System`,
      buttons,
    });
  }
