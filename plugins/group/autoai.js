...
const pluginConfig = {
  ...
  usage:
    ".autoai on/off --haidarmode=<character|custom> --logic=<custom instruction> --type=<text|voice> --mode=<onlychat|assistant>",
  example: ".autoai on --haidarmode=furina --type=voice --mode=onlychat",
  isOwner: false,
  isPremium: false,
  isGroup: true,
  isPrivate: false,
...
` +
  ...
  if (subcmd === "global") {
    ...
    if (globalMode === "on") {
      const modeMatch = fullArgs.match(/--haidarmode=(\w+)/i);
      ...
      const logicMatch = fullArgs.match(
        /--logic=(.+?)(?=\s+--(?:haidarmode|type|logic|mode)|$)/i,
      );
      ...
      if (charKey === "custom" && customLogic) {
        ...
        if (existingGlobal.instruction) {
          ...
          return m.reply(
            `❌ Belum ada persona global yang diset!\n\n> .autoai global on --haidarmode=furina\n> .autoai global on --haidarmode=custom --logic=...`,
          );
        }
      } else {
        const charList = [
  ...
  if (!mode || !["on", "off"].includes(mode)) {
    ...
    txt += `*Penggunaan:*\n`;
    txt += `> .autoai on --haidarmode=<karakter|custom> --type=<text|voice> --mode=<onlychat|assistant>\n`;
    txt += `> .autoai off\n`;
    txt += `> .autoai tambahpersona nama | logic\n`;
    txt += `> .autoai hapuspersona nama\n`;
    ...
    txt += `*Contoh:*\n`;
    txt += `> .autoai on --haidarmode=furina --type=text\n`;
    txt += `> .autoai on --haidarmode=custom --logic=kamu adalah nexa ai\n`;
    txt += `> .autoai tambahpersona nexa | kamu adalah nexa ai\n`;
    txt += `> .autoai global on --haidarmode=furina`;
    return m.reply(txt);
  }
  ...
  if (!charKey) {
    ...
    return m.reply(
      `❌ Karakter tidak valid!\n\n> Karakter tersedia: ${charList}\n\n> Contoh: .autoai on --haidarmode=furina --type=voice\n> Custom: .autoai on --haidarmode=custom --logic=kamu adalah nexa ai`,
    );
  }
  ...
  if (charKey === "custom") {
    if (!customLogic) {
      return m.reply(
        `❌ Mode custom membutuhkan --logic!\n\n> Contoh: .autoai on --haidarmode=custom --logic=kamu adalah nexa ai, ...`,
      );
    }
