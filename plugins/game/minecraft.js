...
let thumbMC = null;
try {
  const p = path.join(process.cwd(), "assets", "images", "haidar-minecraft.jpg");
  if (fs.existsSync(p)) thumbMC = fs.readFileSync(p);
} catch (e) {}

function ctx() {
  const sId = config.saluran?.id || "120363405683815121@newsletter";
  const sName = config.saluran?.name || config.bot?.name || "Haidar-AI";
  return {
