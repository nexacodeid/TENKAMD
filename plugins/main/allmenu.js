...
import { generateWAMessageFromContent, prepareWAMessageMedia, proto } from "haidar";
  ...
  try {
    switch (allmenuVariant) {
      ...
      case 5: {
        ...
        const thumbnail = await _sharp(fs.readFileSync(config.assets["haidar"])).resize(300, 300).toBuffer()
        const qOrder = {
        ...
        const media4 = await prepareWAMessageMedia({ video: fs.readFileSync(config.assets["haidar-mp4"]), gifPlayback: true }, { upload: sock.waUploadToServer });
            ...
            message: {
              ...
              interactiveMessage: {
                ...
                contextInfo: {
                  ...
                  forwardingScore: 9,
                  forwardedNewsletterMessageInfo: {
                    newsletterJid: config.saluran?.id || "120363405683815121@newsletter",
                    newsletterName: config.saluran?.name || config.bot?.name || "Haidar-AI",
                    serverMessageId: 127,
                  },
                },
                nativeFlowMessage: {
                  messageParamsJson: JSON.stringify({
                    ...
                    tap_target_configuration: { title: " X ", description: "bomboclard", canonical_url: "https://haidar.site", domain: "shop.example.com", button_index: 0 },
                  }),
                  buttons: [
