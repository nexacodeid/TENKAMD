...
const fontUrl = getAssetBuffer("haidar-font");
...
async function handler(m, { sock }) {
  ...
  const inputUrl = getAssetBuffer("haidar-kertas");
  if (!inputUrl) {
    return m.reply(
  ...
  try {
    ...
    if (!_fontRegistered) {
      try {
        const fontBuf = getAssetBuffer("haidar-font");
        if (fontBuf) {
    ...
    const bgBuf = getAssetBuffer("haidar-kertas");
