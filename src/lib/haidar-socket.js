...
import { downloadMediaMessage, getContentType } from "haidar";
import { addExifToWebp, DEFAULT_METADATA } from "./haidar-exif.js";
...
import {
  getProfilePicture,
  getProfileBuffer,
} from "./haidar-profile-picture.js";
...
async function extendSocket(sock) {
  ...
  sock.sendStickerPack = async (jid, stickers, m, options = {}) => {
    ...
    const publisher = options.publisher ?? options.author ?? "Haidar-AI";
  ...
  sock.sendButton = async function (
    ...
    if (!options.footer) msg.footer = config.bot?.name || "Haidar-AI";
    if (source) {
      let data = source;
