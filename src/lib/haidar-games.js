import {
  ...
  getRandomReward,
  getProgressiveHint,
} from "./haidar-game-data.js";
import { getDatabase } from "./haidar-database.js";
import { addExpWithLevelCheck } from "./haidar-level.js";
import {
  getGameContextInfo,
  sendGamePreview,
  checkFastAnswer,
} from "./haidar-context.js";
import config from "../../config.js";
import fs from "fs";
import sharp from "sharp";
let fetchBuffer;
try {
  fetchBuffer = (await import("./haidar-utils.js")).fetchBuffer;
} catch { }
        ...
        if (session && session.gameType === gameType) {
          ...
          await sock.sendPreview(
            m.chat,
            {
              ...
              jpegThumbnail: await sharp(fs.readFileSync(config.assets["haidar2"])).resize(300, 300).toBuffer(),
              previewType: 0,
            },
          );
          return;
...
const games = new HaidarGames();

export { HaidarGames, games };
