...
import { addJadibotOwner } from "./haidar-jadibot-database.js";
import { extendSocket } from "./haidar-socket.js";
  ...
  if (usePairing && !state.creds?.registered) {
    try {
      ...
      if (m && m.chat) {
        let thumbnail = null;
        try {
          if (!!getAssetBuffer("haidar2")) {
            thumbnail = getAssetBuffer("haidar2");
          }
        } catch { }
