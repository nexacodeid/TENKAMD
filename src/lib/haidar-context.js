...
const keys = [
  ["haidar-games", (buf) => { gameThumbBuffer = buf; }],
  ["haidar-rpg", (buf) => { rpgThumbBuffer = buf; }],
  ["haidar-winner", (buf) => { winnerThumbBuffer = buf; }],
];
...
function _saluranCtx() {
  const saluranId = config.saluran?.id || "120363405683815121@newsletter";
...
function getRpgContextInfo(title, body) {
  ...
  if (title || body) {
    base.externalAdReply = {
      title: title || config.bot?.name || "Haidar RPG",
      body: body || "",
...
function createFakeQuoted(botName = "Haidar-AI", verified = true) {
  return {
    key: {
      fromMe: false,
