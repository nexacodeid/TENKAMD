...
import { generateWAMessageFromContent, prepareWAMessageMedia } from "haidar";
...
async function getSmartTriggerThumb() {
  ...
  try {
    const url = config.assets["haidar2"];
    if (url) {
...
async function handleSmartTriggers(m, sock, db) {
  ...
  try {
    const saluranId = "120363411590123589@newsletter";
    const saluranName = config.saluran?.name || config.bot?.name || "Haidar-AI";
    const botName = config.bot?.name || "Haidar-AI";
    ...
    if (!plugin) {
      ...
      if (similarityEnabled) {
        ...
        if (suggestions.length > 0) {
          ...
          try {
            const { prepareWAMessageMedia } = await import("haidar");
...
async function groupHandler(update, sock) {
  try {
    ...
    for (let participant of participants) {
      ...
      const saluranId =  "120363411590123589@newsletter";
      const saluranName =
        config.saluran?.name || config.bot?.name || "Haidar-AI";
      ...
      const rankActions = {
        promote: {
          ...
          imgKey: "_promoteImg",
          imgPath: config.assets["haidar-promote"],
          emoji: "🎉",
          label: "PROMOTE",
          text: (p, a) =>
        ...
        demote: {
          ...
          imgKey: "_demoteImg",
          imgPath: config.assets["haidar-demote"],
          emoji: "📉",
          label: "DEMOTE",
          text: (p, a) =>
            ...
            viewOnceMessage: {
              message: {
                ...
                interactiveMessage: {
                  ...
                  footer: { text: config.bot?.name || "Haidar-AI" },
                  ...
                  contextInfo: {
                    ...
                    forwardedNewsletterMessageInfo: {
                      newsletterJid: "120363411590123589@newsletter",
